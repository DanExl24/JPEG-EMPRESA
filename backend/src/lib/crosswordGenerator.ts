 ...
 * Contiene el algoritmo para generar la distribución y coordenadas de un crucigrama.
 */
...
export interface CrosswordPlacedWord extends CrosswordInputWord {
  id: number
  x: number
  y: number
  orientation: 'horizontal' | 'vertical'
...
export interface CrosswordResult {
  success: boolean
  words?: CrosswordPlacedWord[]
  grid?: Record<string, CrosswordCell>
  width?: number
  height?: number
  errorWord?: string
  reason?: string
...
export function generateCrossword(wordsInput: CrosswordInputWord[], layoutMode: string = 'automatic'): CrosswordResult {
  ...
  const placed = new Map<number, { x: number; y: number; orientation: 'horizontal' | 'vertical' }>() 
  ...
  function canPlace(word: { id: number; word: string; clue: string; orientation: 'horizontal' | 'vertical' }, x: number, y: number, orientation: 'horizontal' | 'vertical'): boolean {
  ...
  function place(word: { id: number; word: string; clue: string; orientation: 'horizontal' | 'vertical' }, x: number, y: number, orientation: 'horizontal' | 'vertical'): void {
    const len = word.word.length
    const isHoriz = orientation === 'horizontal'
  ...
  function unplace(word: { id: number; word: string; clue: string; orientation: 'horizontal' | 'vertical' }, x: number, y: number, orientation: 'horizontal' | 'vertical'): void {
    const len = word.word.length
    const isHoriz = orientation === 'horizontal'
  ...
  function solve(): boolean {
    ...
    for (const wordId of Array.from(unplacedIds)) {
  ...
  const resultWords: CrosswordPlacedWord[] = words.map(w => {
  ...
  const normalizedGrid: Record<string, CrosswordCell> = {}
  let width = 0
  let height = 0
...
export function reconstructLayout(words: CrosswordPlacedWord[]): CrosswordResult {
  const grid: Record<string, CrosswordCell> = {}
  let width = 0
  let height = 0
...
function checkConnectivity(words: Array<{ id: number; word: string }>): { connected: boolean; isolated: string[] } {
