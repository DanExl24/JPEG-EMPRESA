...
export function generateCrossword(wordsInput, layoutMode = 'automatic') {
  ...
  const placed = new Map(); // wordId -> { x, y, orientation }
  const grid = new Map(); // "x,y" -> { char, wordIds, wordOrientations, indices }
  ...
  function canPlace(word, x, y, orientation) {
    ...
    for (let i = 0; i < len; i++) {
      ...
      if (cell) {
        // Celda ocupada: debe coincidir la letra y la orientación debe ser cruzada
        if (cell.char !== char) return false;
        if (cell.wordOrientations.includes(orientation)) return false;
      } else {
        // Celda vacía: sus vecinos perpendiculares deben estar vacíos para evitar colisiones adyacentes
  ...
  // Backtracking para buscar una distribución conectada
  ...
  function solve() {
    ...
    for (const wordId of unplacedIds) {
      ...
      // Si es la primera palabra, la colocamos en (0, 0)
      if (placed.size === 0) {
      ...
      // Probar cruzar esta palabra con cualquiera de las ya colocadas
  ...
  // 3. Normalizar coordenadas para que empiecen en (0, 0)
  ...
  // Asignar coordenadas normalizadas a las palabras en su orden original
...
/**
 * Reconstruye el estado del crucigrama utilizando coordenadas y orientaciones pre-guardadas
 */
...
/**
 * Función auxiliar para verificar si el grafo de intersección de palabras es conexo
 */
function checkConnectivity(words) {
  ...
  // Empezar DFS en la primera palabra
  dfs(words[0].id);
